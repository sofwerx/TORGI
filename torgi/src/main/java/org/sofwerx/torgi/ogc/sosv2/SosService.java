...
public class SosService implements SosMessageListener {
    ...
    public void broadcastSensorReadings() {
        ...
        if (sosSensor != null) {
            if (listener != null)
                listener.onError("Cannot send sensor readings as no sensor has been set. Call setSosSensor first");
                    ...
                    if (serverURL == null) {
                        ...
                        try {
                            ...
                            if (responseOperation == null) {
                                Log.e(SosIpcTransceiver.TAG,"Unable to parse reponse from server: "+result);
                                if (listener != null)
                                    listener.onError("Unexpected response from SOS server");
                            } else
                                onOperationReceived(responseOperation);
                        } catch (IOException | TransformerException | ParserConfigurationException e) {
                            if (listener != null)
                                listener.onError("Unable to connect to SOS server: "+e.getMessage());
                        }
                    }
                } else {
                    if (listener != null)
                        listener.onError("Cannot send SOS messages as the SosService has not be enabled (call setOn())");
                }
            });
        } else {
            Log.d(SosIpcTransceiver.TAG,"...but handler is not yet ready");
            if (listener != null)
                listener.onError("Unable to broadcast yet as the thread and handler SosService need are not yet ready");
        }
    }
            ...
            if (sosSensor != null) {
                ...
                if ((response.getAcceptedTemplate() != null) && (sosSensor.getAssignedProcedure() != null)
                    ...
                    if (sendSensorReadingWhenReady.get())
                        broadcastSensorReadings();
                    if (listener != null)
                        listener.onConfigurationSuccess();
                } else
